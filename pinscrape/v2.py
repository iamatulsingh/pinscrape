...
class Pinterest:
    def __init__(
        self, user_agent: str = "", proxies: dict = None, sleep_time: float = None
    ):
        self.errors = []
        self.user_agent = (
    ...
    def get_pin_details(self, username: str, board: str) -> dict:
        ...
        params = {
            ...
            "data": '{"options":{"field_set_key":"profile","username":"'
            + username
            + '"},"context":{}}',
    ...
    def download(
        self, url_list: list, number_of_workers: int, output_folder: str
    ) -> None:
    ...
    def fetch_cookies(self) -> str:
        cookie_res = requests.request(
            "GET", f"{self.BASE_URL}/ideas/", data={}, proxies=self.proxies
        )
        if cookie_res.status_code != 200:
            logging.error(
                f"Failed attempt to get Cookies. Status code for cookie is {cookie_res.status_code}"
            )
            exit()
    ...
    def search(self, query: str, page_size=26) -> list:
        ...
        ts = self.time_epoch
        url = (
            f"{self.BASE_URL}/resource/BaseSearchResource/get/?source_url={quote_plus(source_url)}"
            f"&data={data}&_={ts}"
        )
        payload = {}
        ...
        response = self.session.get(
            url, headers=headers, data=payload, proxies=self.proxies
        )
        ...
        if response.status_code != 200:
            logging.warning(
                f"Image search has failed!, {response.status_code}, {response.text}"
            )
            self.errors.append(
                f"Image search has failed!, {response.status_code}, {response.text}"
            )
        ...
        try:
            ...
            results = (
                json_data.get("resource_response", {})
                .get("data", {})
                .get("results", [])
            )
